...
module CostQuery::UserCustomFieldMixin
  ...
  def generate_subclasses
    UserCustomField.where(field_format: SQL_TYPES.keys).map do |field|
      class_name = "UserCustomField#{field.id}"
  ...
  def remove_subclasses
    module_parent.constants.each do |constant|
      if constant.to_s.match /^UserCustomField\d+/
  ...
  def list_join_table(field)
    ...
    ON #{cf_name}.customized_type = 'User'

    AND #{cf_name}.custom_field_id = #{field.id}
    AND #{cf_name}.customized_id = entries.user_id
  ...
  def default_join_table(field)
    ...
    \t\t%1$s)
    AS %2$s
    ON %2$s.customized_type = 'User'
    AND %2$s.custom_field_id = %3$d
    AND %2$s.customized_id = entries.user_id
